...
# RFGSM攻击代码
def rfgsm_attack(image, epsilon, data_grad):
    x0 = image + torch.empty_like(image).uniform_(-epsilon, epsilon)
    x0 = torch.clamp(x0, 0, 1)
    
    sign_grad = data_grad.sign()
    adv = x0 + epsilon * sign_grad
...
def test(model, device, test_loader, epsilon):
    ...
    for data, target in test_loader:
        ...
        perturbed_data = rfgsm_attack(data, epsilon, data_grad)
