...
def mim_attack(model, image, target, epsilon, num_iter=10, alpha=None, mu=1.0, device='cpu'):
    if alpha is None:
    ...
    for _ in range(num_iter):
        ...
        eta = torch.clamp(x_adv - image, min=-epsilon, max=epsilon)
        x_adv = torch.clamp(image + eta, 0, 1).detach()
